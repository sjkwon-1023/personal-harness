#!/usr/bin/env python3
"""agents.json의 CLI별 설정과 <이름>.md 본문으로 각 CLI의 서브에이전트 파일을 만든다.

CLI 설정에 model이 없으면 모델 줄을 쓰지 않아 그 CLI의 메인 모델을 이어받는다.
"""
import json
...
HOME = Path.home()
NOTICE = "~/.config/coding-harness/agents에서 생성한 파일이다. 원본을 고친 뒤 sync.py를 다시 실행한다."
...
def codex(name, agent, body):
    ...
    lines += [f"sandbox_mode = {quoted(settings['sandbox'])}",
              f"description = {quoted(agent['description'])}",
              f"developer_instructions = '''\n{body}'''"]
    return HOME / ".codex/agents" / f"{name}.toml", "\n".join(lines) + "\n"


def opencode(name, agent, body):
    settings = agent["opencode"]
    lines = ["---", f"# {NOTICE}", f"description: {quoted(agent['description'])}", "mode: subagent"]
    if "model" in settings:
    ...
    for name, agent in agents.items():
        ...
        for cli, (render, root) in RENDERERS.items():
            if cli not in agent or not (HOME / root).is_dir():
                continue
            path, text = render(name, agent, body)
