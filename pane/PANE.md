...
python3 $H prepare --role <역할> --model <별칭> --target-tab <ID> \
...
- `--model`: `routing.json`의 별칭. 별칭이 CLI와 모델 ID를 함께 정한다. 별칭에 없는 모델은 `--cli claude|codex|opencode|agy`와 그 CLI의 정확한 모델 ID를 준다.
